...
/**
 * The modification of QLearningAgent from
 * Artificial Intelligence A Modern Approach (3rd Edition): page 844.
 * @author Dongsoo Seo
 */
public class DoubleQLearningAgent<S, A extends Action> extends ReinforcementAgent<S, A> {
    // persistent: Q, a table of action values indexed by state and action,
    // initially zero

    private Map<Pair<S, A>, Double> Qa = new HashMap<>(), Qb = new HashMap<>();
     ...
     */
    public DoubleQLearningAgent(ActionsFunction<S, A> actionsFunction, double alpha, double gamma, int Ne, double Rplus) {
    ...
    public A execute(PerceptStateReward<S> percept) {
        ...
        if (isTerminal(sPrime)) {
            Qa.put(new Pair<>(sPrime, null), rPrime);
            Qb.put(new Pair<>(sPrime, null), rPrime);
        }
        ...
        if (null != s) {
            ...
            if (random.nextBoolean()) {
                ...
                if (isTerminal(sPrime)) {
                    ...
                    s = sPrime;
                    a = argmaxAPrime(sPrime, Qa);
                    r = rPrime;
                }
            } else {
                update(Qb, Qa, sa, sPrime);
                if (isTerminal(sPrime)) {
                    s = null;
                    a = null;
                    r = null;
    ...
    private void update(Map<Pair<S, A>, Double> Q1, Map<Pair<S, A>, Double> Q2, Pair<S, A> sa, S sPrime) {
        Double Q_sa = Q1.get(sa);
        if (null == Q_sa) {
            Q_sa = 0.0;
        }
        Q1.put(sa, Q_sa + alpha(Nsa, s, a) * (r + gamma * maxAPrime(sPrime, Q2) - Q_sa));
    }
    ...
    public Map<S, Double> getUtility() {
        ...
        Map<S, Double> Ua = getUtility(Qa);
        Map<S, Double> Ub = getUtility(Qb);
        Map<S, Double> U = new HashMap<>(Ua);
        Ub.keySet().forEach((s) -> {
            if (U.containsKey(s)) {
                U.put(s, (U.get(s) + Ub.get(s)) / 2);
            } else {
                U.put(s, Ub.get(s) / 2);
            }
        });
    ...
    private Map<S, Double> getUtility(Map<Pair<S, A>, Double> Q) {
    ...
    private double maxAPrime(S sPrime, Map<Pair<S, A>, Double> Q) {
    ...
    private A argmaxAPrime(S sPrime, Map<Pair<S, A>, Double> Q) {
        A a = null;
        double max = Double.NEGATIVE_INFINITY;
        for (A aPrime : actionsFunction.actions(sPrime)) {
